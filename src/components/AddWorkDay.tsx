...
import type { WorkDay, Employer, WorkSegment } from '@/lib/api'
import { calculateMultiSegmentPayment, formatCurrency } from '@/lib/calculations'
...
export function AddWorkDay({ employers, workDays, onSave, editingId, onCancel }: AddWorkDayProps) {
  const isEditing = Boolean(editingId)

  const existingWorkDay = useMemo(() => {
    if (editingId) {
      return workDays.find(w => w.id === editingId)
    }
    return null
  }, [editingId, workDays])
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 transition-colors">
              ...
              <div key={segment.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 md:p-4">
                ...
                {selectedEmployer && selectedEmployer.bonuses && selectedEmployer.bonuses.length > 0 && (
                  <div className="col-span-2 lg:col-span-4 mt-2 md:mt-4 pt-3 border-t border-gray-200 dark:border-gray-600">
                    <h5 className="text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">תוספות לנקודה זו:</h5>
                    <div className="flex flex-wrap gap-2 md:gap-3">
                      {selectedEmployer.bonuses.map((bonus) => (
                        <label
                          key={bonus.id}
                          className="flex items-center gap-1.5 md:gap-2 cursor-pointer bg-white dark:bg-gray-600 px-2 md:px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-500 hover:border-blue-300"
                        >
                          <input
                            type="checkbox"
                            ...
                            className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                          />
                          <span className="text-xs md:text-sm text-gray-700 dark:text-gray-200">{bonus.name}</span>
        ...
        {calculations && (
          <div className="mt-4 md:mt-6 bg-gray-50 dark:bg-gray-700 rounded-lg p-3 md:p-4">
            <h3 className="font-medium text-gray-900 dark:text-white mb-2 md:mb-3 text-sm md:text-base">סיכום</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 text-xs md:text-sm">
              <div>
                <span className="text-gray-500 dark:text-gray-400">שעות רגילות:</span>
                <p className="font-medium text-gray-900 dark:text-white">{calculations.regularHours} שעות</p>
              </div>
              <div>
                <span className="text-gray-500 dark:text-gray-400">שעות נוספות:</span>
                <p className="font-medium text-gray-900 dark:text-white">{calculations.overtimeHours} שעות</p>
              </div>
              <div>
                <span className="text-gray-500 dark:text-gray-400">לפני מע&quot;מ:</span>
                <p className="font-medium text-gray-900 dark:text-white">{formatCurrency(calculations.totalBeforeVat)}</p>
              </div>
              <div>
