  ...
  return (
    <div className="pb-4">
      ...
      {filteredWorkDays.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8 text-center text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 transition-colors">
          אין נתונים להצגה
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4 transition-colors">
              <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">ימי עבודה</p>
              <p className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{stats.workDaysCount}</p>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4 transition-colors">
              <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">שעות עבודה</p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4 transition-colors">
              <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">קילומטרים</p>
              <p className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{stats.totalKm}</p>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4 transition-colors">
              <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">סה"כ הכנסות</p>
          ...
          {employerStats.length > 1 && (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-4 transition-colors">
              <h3 className="font-medium text-gray-900 dark:text-white dark:text-white mb-3 md:mb-4 text-sm md:text-base">פירוט לפי מעסיק</h3>
              <div className="space-y-2 md:space-y-3">
                {employerStats.map((stat, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">{stat.name}</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{stat.days} ימים</p>
                    </div>
