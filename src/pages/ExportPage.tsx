...
import type { WorkDay, Employer, BusinessDetails } from '../types';
          ...
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 md:p-4 mb-4 md:mb-6">
            ...
            <div className="md:hidden space-y-2">
              {filteredWorkDays.map(day => (
                <div key={day.id} className="bg-gray-50 dark:bg-gray-700 dark:bg-gray-700 rounded-lg p-3 flex justify-between items-center">
                  <div>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredWorkDays.map(day => (
                    <tr key={day.id}>
                      ...
                      <td className="px-3 py-2">{day.location || '-'}</td>
                      <td className="px-3 py-2">{formatHours(day.regularHours)}</td>
                      <td className="px-3 py-2">{day.overtimeHours > 0 ? formatHours(day.overtimeHours) : '-'}</td>
                      <td className="px-3 py-2">{day.kilometers || '-'}</td>
                      <td className="px-3 py-2 font-medium">{formatCurrency(day.totalWithVat)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot className="bg-gray-50 dark:bg-gray-700 font-medium">
                  <tr>
                    <td colSpan={2} className="px-3 py-2">סה"כ</td>
                    <td className="px-3 py-2">{formatHours(totals.hours)}</td>
                    <td className="px-3 py-2">-</td>
                    <td className="px-3 py-2">{totals.km}</td>
                    <td className="px-3 py-2">{formatCurrency(totals.withVat)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
            ...
            <div className="mt-3 md:mt-4 grid grid-cols-2 gap-2 md:gap-4 text-xs md:text-sm bg-blue-50 dark:bg-blue-900/30 p-3 rounded">
              <div>
                ...
                <span className="font-medium mr-1 md:mr-2">{formatCurrency(totals.beforeVat)}</span>
              </div>
              <div>
